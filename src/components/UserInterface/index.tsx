...
import { useDecompressor } from "hooks/decompressor";
...
const UserInterface = () => {
  ...
  const { init } = useDecompressor();

  const handleAddRoadClick = () => {
    setCurrUISelection(
      currUISelection !== UISelection.ADDROAD ? UISelection.ADDROAD : null
    );
  };

  const handleEditRoadClick = () => {
    setCurrUISelection(
      currUISelection !== UISelection.EDITROAD ? UISelection.EDITROAD : null
    );
  };

  const handleSculptClick = () => {
    setCurrUISelection(
      currUISelection !== UISelection.SCULPT ? UISelection.SCULPT : null
    );
  };
  ...
  return (
    <div className="fixed top-0 left-0 z-10 flex flex-col w-full">
      ...
      {(currUISelection === UISelection.ADDROAD ||
        currUISelection === UISelection.EDITROAD) && <EditTrackModal />}
    </div>
  );
};
